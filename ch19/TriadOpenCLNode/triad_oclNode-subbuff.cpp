...
mfn_t dispatch_node(g, tbb::flow::unlimited, [&]( const float& offloadRatio, mfn_t::output_ports_type &ports ) {
  ...
  std::get<0>(ports).try_put(Adevice.subbuffer(0,ceil(vsize*offloadRatio)));
  std::get<1>(ports).try_put(Bdevice.subbuffer(0,ceil(vsize*offloadRatio)));
  std::get<2>(ports).try_put(tbb::flow::opencl_subbuffer<cl_float>(Cdevice,0,ceil(vsize*offloadRatio)));
