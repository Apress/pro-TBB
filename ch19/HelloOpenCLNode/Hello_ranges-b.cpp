...
int main(int argc, const char* argv[]) {
  ...
  //GPU node:
  gpu_device_selector2 s;
  tbb::flow::opencl_program<> program(std::string("printranges.cl"));
  // //Option 1:
  // opencl_node<tuple<int>> gpu_node(g, program.get_kernel("cl_print"),s);
  // gpu_node.set_range({{rows,cols},{rows/2,cols/2}});
  // gpu_node.set_args(Adevice,Bdevice,Cdevice,port_ref<0>);
  // tbb::flow::input_port<0>(gpu_node).try_put( rows );

  //Option 2:
  // using NDRange = std::array<int,2> ;
  // opencl_node<tuple<NDRange>> gpu_node(g, program.get_kernel("cl_print"),s);
  // gpu_node.set_range(port_ref<0>);
  // gpu_node.set_args(Adevice,Bdevice,Cdevice,rows);
  // tbb::flow::input_port<0>(gpu_node).try_put({{rows,cols}});//,{rows/2,cols/2}});

  //Option 3:
  //tbb::flow::opencl_range r({{rows,cols},{rows/2,cols/2}});
  opencl_node<tuple<tbb::flow::opencl_range>> gpu_node(g, program.get_kernel("cl_print"),s);
  gpu_node.set_range(port_ref<0>);
  gpu_node.set_args(Adevice,Bdevice,Cdevice,rows);
  tbb::flow::input_port<0>(gpu_node).try_put(tbb::flow::opencl_range({{rows,cols},{rows/2,cols/2}}));
